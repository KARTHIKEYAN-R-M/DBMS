...
%>
   ...
   if(ab.equals("ord"))
    ...
    ResultSet rs=stmt.executeQuery("select count(o_id) from ord where id='"+a+"'");
out.println("Number of orders in your restaurant:");
         
   while (rs.next()) {
            String id = rs.getString("count(o_id)");
            out.println(id);%><br><%
         }}

/* if(ab.equals("b")){  
    Statement stmt=con.createStatement();
    ResultSet rs=stmt.executeQuery("select count(o_id) from ord where id='"+id+"'");
